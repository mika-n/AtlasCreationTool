...
//-----------------------------------------------------------------------------
#pragma warning(push)
#pragma warning(disable:26812) // unscoped enum usage (ie. not a enum class)
...
//-----------------------------------------------------------------------------
static CD3DApplication* g_pD3DApp = nullptr;
                                  ...
                                  LPARAM lParam )
{
    switch( uMsg )
    {
        ...
        case WM_SETCURSOR:
         ...
         case WM_MOUSEMOVE:
            if( m_bActive && m_pd3dDevice != nullptr )
            {
                POINT ptCursor;
...
INT CD3DApplication::Run()
{
    ...
    HACCEL hAccel = LoadAccelerators( nullptr, MAKEINTRESOURCE(IDR_MAIN_ACCEL) );
    ...
    while( WM_QUIT != msg.message  )
    {
        ...
        if( bGotMsg )
        {
            ...
            if( hAccel == nullptr || m_hWnd == nullptr || 
...
void CD3DApplication::UpdateStats()
{
    ...
    if( fTime - fLastTime > 1.0f )
    {
        ...
        {
            _sntprintf_s( strFmt, 100, TEXT("backbuf %s, adapter %s"), 
...
HRESULT CD3DApplication::DisplayErrorMsg( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        case D3DAPPERR_NODIRECT3D:
            strcpy_s( strMsg, _T("Could not initialize Direct3D. You may\n")
        ...
        case D3DAPPERR_NOCOMPATIBLEDEVICES:
            strcpy_s( strMsg, _T("Could not find any compatible Direct3D\n")
                             _T("devices.") );
            break;

        case D3DAPPERR_NOWINDOWABLEDEVICES:
            strcpy_s( strMsg, _T("This sample cannot run in a desktop\n")
        ...
        case D3DAPPERR_NOHARDWAREDEVICE:
            strcpy_s( strMsg, _T("No hardware-accelerated Direct3D devices\n")
                             _T("were found.") );
            break;

        case D3DAPPERR_HALNOTCOMPATIBLE:
            strcpy_s( strMsg, _T("This sample requires functionality that is\n")
                             _T("not available on your Direct3D hardware\n")
                             _T("accelerator.") );
            break;

        case D3DAPPERR_NOWINDOWEDHAL:
            strcpy_s( strMsg, _T("Your Direct3D hardware accelerator cannot\n")
        ...
        case 0x80070002: // HRESULT_FROM_WIN32( ERROR_FILE_NOT_FOUND ):
            strcpy_s( strMsg, _T("Could not load required media." ) );
            break;

        case D3DAPPERR_RESETFAILED:
            strcpy_s( strMsg, _T("Could not reset the Direct3D device." ) );
            break;

        case D3DAPPERR_NONZEROREFCOUNT:
            strcpy_s( strMsg, _T("A D3D object has a non-zero reference\n")
                             _T("count (meaning things were not properly\n")
                             _T("cleaned up).") );
            break;

        case D3DAPPERR_NULLREFDEVICE:
            strcpy_s( strMsg, _T("Warning: Nothing will be rendered.\n")
        ...
        case E_OUTOFMEMORY:
            strcpy_s( strMsg, _T("Not enough memory.") );
            break;

        case D3DERR_OUTOFVIDEOMEMORY:
            strcpy_s( strMsg, _T("Not enough video memory.") );
            break;

        case D3DERR_DRIVERINTERNALERROR:
            strcpy_s( strMsg, _T("A serious problem occured inside the display driver.") );
            dwType = MSGERR_APPMUSTEXIT;
            break;

        default:
            strcpy_s( strMsg, _T("Generic application error. Enable\n")
    ...
    if( MSGERR_APPMUSTEXIT == dwType )
    {
        ...
        //MessageBox( nullptr, strMsg, m_strWindowTitle, MB_ICONERROR|MB_OK );
    ...
    else
    {
        ...
        MessageBox( nullptr, strMsg, m_strWindowTitle, MB_ICONWARNING|MB_OK );
    }
