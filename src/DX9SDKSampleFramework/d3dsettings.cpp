...
CD3DSettingsDialog* s_pSettingsDialog = nullptr;
...
{
    return DialogBox( nullptr, MAKEINTRESOURCE( IDD_SELECTDEVICE ), 
...
void CD3DSettingsDialog::WindowedFullscreenChanged( void )
{
    D3DAdapterInfo* pAdapterInfo = (D3DAdapterInfo*)ComboBoxSelected( IDC_ADAPTER_COMBO );
    D3DDeviceInfo* pDeviceInfo = (D3DDeviceInfo*)ComboBoxSelected( IDC_DEVICE_COMBO );
    if( pAdapterInfo == nullptr || pDeviceInfo == nullptr )
        return;
    ...
    if( IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        _snprintf_s( strResolution, 50, TEXT("%d by %d"), m_d3dSettings.Windowed_DisplayMode.Width, 
            m_d3dSettings.Windowed_DisplayMode.Height );
        strResolution[49] = 0;
        ...
        if( m_d3dSettings.Windowed_DisplayMode.RefreshRate == 0 )
            ...
            _snprintf_s( strRefreshRate, 50, TEXT("%d Hz"), m_d3dSettings.Windowed_DisplayMode.RefreshRate );
        strRefreshRate[49] = 0;
    ...
    if( !IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
        {
            ...
            if (displayMode.Format == adapterFormat)
            {
                ...
                _snprintf_s( strResolution, 50, TEXT("%d by %d"), displayMode.Width, displayMode.Height );
                strResolution[49] = 0;
    ...
    for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
    {
            ...
            displayMode.Height == height)
        {
            ...
            if( displayMode.RefreshRate == 0 )
                ...
                _snprintf_s( strRefreshRate, 50, TEXT("%d Hz"), displayMode.RefreshRate );
            strRefreshRate[49] = 0;
    ...
    for( UINT idc = 0; idc < pDeviceInfo->pDeviceComboList->Count(); idc++ )
    {
            ...
            pDeviceCombo->BackBufferFormat == backBufferFormat)
        {
            ...
            else
            {
                ...
                ComboBoxAdd( IDC_DEPTHSTENCILBUFFERFORMAT_COMBO, nullptr, TEXT("(not used)") );
                ComboBoxSelectIndex( IDC_DEPTHSTENCILBUFFERFORMAT_COMBO, 0 );
            }
