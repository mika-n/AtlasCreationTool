                                 ...
                                 BOOL bAddWatermark )
{
    ...
    if( FAILED( hr = pd3dDevice->CreateOffscreenPlainSurface( dwWidth, dwHeightDest, 
        D3DFMT_A8R8G8B8, D3DPOOL_SCRATCH, &pCursorSurface, nullptr ) ) )
    {
        goto End;
    }
...
LPCTSTR D3DUtil_D3DFormatToString( D3DFORMAT format, bool bWithPrefix )
{
    ...
    if( bWithPrefix || _tcsstr( pstr, TEXT("D3DFMT_") )== nullptr )
        return pstr;
    else
...
{
    if( nullptr == pvEyePt || nullptr == pvLookatPt )
        return;
    ...
    D3DXMATRIX mInvView;
    D3DXMatrixInverse( &mInvView, nullptr, &m_mView );
...
VOID CFirstPersonCamera::FrameMove( FLOAT fElapsedTime )
{
    ...
    D3DXMatrixInverse( &m_mCameraWorld, nullptr, &m_mView );
}
