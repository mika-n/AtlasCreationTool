...
static CD3DScreensaver* s_pD3DScreensaver = nullptr;
...
CD3DScreensaver::CD3DScreensaver()
{
    ...
    m_hPasswordDLL = nullptr;
    m_hWnd = nullptr;
    m_VerifySaverPassword = nullptr;
    ...
    m_fElapsedTime = 0.0f;
    m_hInstance = nullptr;
    m_SaverMode = SaverMode::sm_config;
}
...
*/
    ...
    switch ( m_SaverMode )
    {
        ...
        case sm_full:
            // Create windows for each monitor.  Note that m_hWnd is nullptr when CreateWindowEx
            ...
            for( DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++ )
            {
                ...
                if( pMonitorInfo->hMonitor == nullptr )
                    continue;
                ...
                pMonitorInfo->hWnd = CreateWindowEx( WS_EX_TOPMOST, TEXT("D3DSaverWndClass"), 
                    ...
                    rc.bottom - rc.top, m_hWnd, nullptr, m_hInstance, this );
                if( pMonitorInfo->hWnd == nullptr )
                    return E_FAIL;
                if( m_hWnd == nullptr )
...
#pragma warning(push)
#pragma warning(disable:28159)  // Deprecated GetVersionEx
...
#pragma warning(pop)
    ...
    if ( m_SaverMode == sm_full && m_bIs9x )
    {
        ...
        HKEY hKey; 
        if ( RegCreateKeyEx( HKEY_CURRENT_USER, REGSTR_PATH_SCREENSAVE, 0, nullptr, 0, KEY_READ, nullptr, &hKey, nullptr ) == ERROR_SUCCESS ) 
        { 
            DWORD dwVal;
            DWORD dwSize = sizeof(dwVal); 
 
            if ( (RegQueryValueEx( hKey, REGSTR_VALUE_USESCRPASSWORD, nullptr, nullptr,
    ...
    {
        bGotMsg = PeekMessage( &msg, nullptr, 0, 0, PM_REMOVE );
        if( bGotMsg )
        {
            TranslateMessage( &msg );
            DispatchMessage( &msg );
...
LRESULT CD3DScreensaver::SaverProc( HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam )
{
    switch ( uMsg )
        ...
        case WM_POWERBROADCAST:
            if( wParam == PBT_APMSUSPEND && m_VerifySaverPassword == nullptr )
                InterruptSaver();
            break;
...
VOID CD3DScreensaver::InterruptSaver()
{
        ...
        m_SaverMode == sm_full && !m_bCheckingSaverPassword )
    {
        if( m_bIs9x && m_SaverMode == sm_full )
        {
            ...
            // or we're not on 9x. 
            if ( m_VerifySaverPassword != nullptr )
            {
...
HRESULT CD3DScreensaver::CreateFullscreenRenderUnit( RenderUnit* pRenderUnit )
{
    ...
    if( SUCCEEDED( hr ) && bAtLeastOneFailure && m_strRegPath[0] != TEXT('\0') )
    {
        ...
        if( ERROR_SUCCESS == RegCreateKeyEx( HKEY_CURRENT_USER, m_strRegPath, 
            0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hkeyParent, nullptr ) )
        {
            wsprintf( strKey, TEXT("Screen %d"), iMonitor + 1 );
            if( ERROR_SUCCESS == RegCreateKeyEx( hkeyParent, strKey, 
                0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hkey, nullptr ) )
            {
                RegSetValueEx( hkey, TEXT("Width"), 0, REG_DWORD, 
                    (BYTE*)&pD3DAdapterInfo->dwUserPrefWidth, sizeof(DWORD) );
                RegSetValueEx( hkey, TEXT("Height"), 0, REG_DWORD, 
                    (BYTE*)&pD3DAdapterInfo->dwUserPrefHeight, sizeof(DWORD) );
                RegSetValueEx( hkey, TEXT("Format"), 0, REG_DWORD, 
                    (BYTE*)&pD3DAdapterInfo->d3dfmtUserPrefFormat, sizeof(DWORD) );
                RegSetValueEx( hkey, TEXT("Adapter ID"), 0, REG_BINARY, 
...
VOID CD3DScreensaver::SwitchToRenderUnit( UINT iRenderUnit )
{
    ...
    if( m_pd3dDevice != nullptr && !m_bErrorMode )
    {
...
HRESULT CD3DScreensaver::BuildDeviceList()
{
    ...
    for( UINT iAdapter = 0; iAdapter < m_pD3D->GetAdapterCount(); iAdapter++ )
    {
        ...
        if( m_Adapters[m_dwNumAdapters] == nullptr )
        {
        ...
        // is disabled, the adapter has a nullptr HMONITOR and we cannot find the 
        // corresponding MonitorInfo.  (Well, if one monitor was disabled, we
        // could link the one MonitorInfo with a nullptr HMONITOR to the one
        // D3DAdapterInfo with a nullptr HMONITOR, but if there are more than one,
    ...
    for( DWORD iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++ )
    {
        ...
        if( pMonitorInfo->widthError == 0 )
        {
            if( m_SaverMode == sm_preview )                
            {
                ...
                InvalidateRect( hwnd, nullptr, FALSE );    // Invalidate the hwnd so it gets drawn
                UpdateWindow( hwnd );
            }
            else
            {
                                       ...
                                       DWORD dwNumChars )
{
    ...
    LoadString( nullptr, resid, pszError, dwNumChars );
...
VOID CD3DScreensaver::UpdateFrameStats()
{
    ...
    if( fTime - fLastTime > 1.0f )
    {
        ...
        for( iRenderUnit = 0; iRenderUnit < m_dwNumRenderUnits; iRenderUnit++ )
        {
            ...
            sprintf_s( pRenderUnit->strFrameStats, TEXT("%.02f fps (%dx%dx%d)"), m_fFPS,
...
VOID CD3DScreensaver::DoPaint(HWND hwnd, HDC hdc)
{
    ...
    MonitorInfo* pMonitorInfo = nullptr;
...
VOID CD3DScreensaver::ChangePassword()
{
    ...
    if ( mpr != nullptr )
    {
        ...
        if ( pwd != nullptr )
            pwd( "SCRSAVE", m_hWndParent, 0, nullptr );
...
VOID CD3DScreensaver::ReadScreenSettings( HKEY hkeyParent )
{
    ...
    RegQueryValueEx( hkeyParent, TEXT("AllScreensSame"), nullptr, &dwType, 
    ...
    for( iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++ )
    {
            ...
            0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, nullptr, &hkey, nullptr ) )
        {
            ...
            RegQueryValueEx( hkey, TEXT("Adapter ID"), nullptr, &dwType, 
                (BYTE*)&guidAdapterID, &dwLength);

            dwLength = sizeof(DWORD);
            RegQueryValueEx( hkey, TEXT("Leave Black"), nullptr, &dwType, 
            ...
            {
                dwLength = sizeof(DWORD);
                RegQueryValueEx( hkey, TEXT("Disable Hardware"), nullptr, &dwType, 
                    (BYTE*)&pD3DAdapterInfo->bDisableHW, &dwLength);
                dwLength = sizeof(DWORD);
                RegQueryValueEx( hkey, TEXT("Width"), nullptr, &dwType, 
                    (BYTE*)&pD3DAdapterInfo->dwUserPrefWidth, &dwLength);
                dwLength = sizeof(DWORD);
                RegQueryValueEx( hkey, TEXT("Height"), nullptr, &dwType, 
                    (BYTE*)&pD3DAdapterInfo->dwUserPrefHeight, &dwLength);
                dwLength = sizeof(DWORD);
                RegQueryValueEx( hkey, TEXT("Format"), nullptr, &dwType, 
...
VOID CD3DScreensaver::WriteScreenSettings( HKEY hkeyParent )
{
    ...
    RegSetValueEx( hkeyParent, TEXT("AllScreensSame"), 0, REG_DWORD, 
    ...
    for( iMonitor = 0; iMonitor < m_dwNumMonitors; iMonitor++ )
    {
        ...
        wsprintf( strKey, TEXT("Screen %d"), iMonitor + 1 );
        if( ERROR_SUCCESS == RegCreateKeyEx( hkeyParent, strKey, 
            0, nullptr, REG_OPTION_NON_VOLATILE, KEY_ALL_ACCESS, 0, &hkey, nullptr ) )
        {
            RegSetValueEx( hkey, TEXT("Leave Black"), 0, REG_DWORD, 
                (BYTE*)&pD3DAdapterInfo->bLeaveBlack, sizeof(DWORD) );
            RegSetValueEx( hkey, TEXT("Disable Hardware"), 0, REG_DWORD, 
    ...
    case WM_INITDIALOG:
        {
            ...
            for( iAdapter = 0; iAdapter < m_dwNumAdapters; iAdapter++ )
            {
                ...
                if( s_AdaptersSave[iAdapter] != nullptr )
...
#pragma warning(push)
#pragma warning(disable:26454) // TCN_SELCHANGE negative value warning
    case WM_NOTIFY:
        {
        ...
        case IDC_MOREINFO:
            {
                ...
                {
                    // Good HAL
                    LoadString( nullptr, IDS_INFO_GOODHAL, szText, 500 );
                }
                else if( bHasHAL && bDisabledHAL )
                {
                    // Disabled HAL
                    if( bHasSW && bHasAppCompatSW )
                        LoadString( nullptr, IDS_INFO_DISABLEDHAL_GOODSW, szText, 500 );
                    else if( bHasSW )
                        LoadString( nullptr, IDS_INFO_DISABLEDHAL_BADSW, szText, 500 );
                    else 
                        LoadString( nullptr, IDS_INFO_DISABLEDHAL_NOSW, szText, 500 );
                }
                else if( bHasHAL && !bHasAppCompatHAL )
                {
                    // Bad HAL
                    if( bHasSW && bHasAppCompatSW )
                        LoadString( nullptr, IDS_INFO_BADHAL_GOODSW, szText, 500 );
                    else if( bHasSW )
                        LoadString( nullptr, IDS_INFO_BADHAL_BADSW, szText, 500 );
                    else 
                        LoadString( nullptr, IDS_INFO_BADHAL_NOSW, szText, 500 );
                }
                else 
                {
                    // No HAL
                    if( bHasSW && bHasAppCompatSW )
                        LoadString( nullptr, IDS_INFO_NOHAL_GOODSW, szText, 500 );
                    else if( bHasSW  )
                        LoadString( nullptr, IDS_INFO_NOHAL_BADSW, szText, 500 );
                    else 
                        LoadString( nullptr, IDS_INFO_NOHAL_NOSW, szText, 500 );
                }
...
VOID CD3DScreensaver::SetupAdapterPage( HWND hWnd )
{
    ...
    if( bHasHAL && !bDisabledHAL && bHasAppCompatHAL )
    {
        LoadString( nullptr, IDS_RENDERING_HAL, szStatus, 200 );
    }
    else if( bHasSW && bHasAppCompatSW )
    {
        LoadString( nullptr, IDS_RENDERING_SW, szStatus, 200 );
    }
    else
    {
        LoadString( nullptr, IDS_RENDERING_NONE, szStatus, 200 );
    }
