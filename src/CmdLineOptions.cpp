...
{
    if ((option < CLO_NUM && option >= 0) && (n < kNumArguments[option]))
            ...
            for (j = 0; j < CLO_NUM; ++j)
                if (!_strcmpi(kParseString[j], argv[i]))         // is it a known cmd-line option?
                {
                    if (mCurrent[j].present)                    //yes: was it set before?
                    {
                        ...
                        sprintf_s( warning, "Option \'%s\' specified multiple times on the cmd-line:",
...
bool CmdLineOptionCollection::IsArgPowerOf2(eCmdLineOptionType option) const
{
    ...
    int         resolution;    
    int const   kError = sscanf_s(mCurrent[option].pStartArgs[0], "%i", &resolution);
    
    if ((kError != 1) || (resolution < 2))
    {
        sprintf_s(string, "%s option requires argument to be an integer greater than 1.", kShortDescription[option]);
    ...
    if (kRemainder != 0.0f)
    {
        sprintf_s(string, "%s option requires argument to be a power of 2.", kShortDescription[option]);
        return PrintError(string);
    }
    return true;
}
...
bool    CmdLineOptionCollection::Check() const
{
    ...
    if (mNumFilenames < 1)
        return PrintError("No source image filenames specified.");
    ...
    {
        sprintf_s(string, "%s currently uses the passed in texture's width and thus ignores %s", 
    ...
    {
        sprintf_s(string, "%s currently uses the passed in texture's height and thus ignores %s", 
    ...
    {
        sprintf_s(string, "%s specified, yet %s is not specified: ignoring %s", 
    ...
    {
        sprintf_s(string, "%s option requires %s option to be set.", kShortDescription[CLO_VOLUME], kShortDescription[CLO_NOMIPMAP]);
        return PrintError(string);
    }
    ...
    {
        sprintf_s(string, "%s option has to be set.", kShortDescription[CLO_OUTFILE]);
        return PrintError(string);
    }
...
void CmdLineOptionCollection::PrintUsage() const
{
	...
	fprintf( stderr, "\n");
	fprintf( stderr, "Usage: AtlasCreationTool.exe");
    ...
    fprintf( stderr, " <img1> <img2> <img3> ...\n\n");
    
    for (i = 0; i < CLO_NUM; ++i)
    {
        if (i != CLO_HELP && i != CLO_HELP_ALTERNATE1 && i != CLO_HELP_ALTERNATE2)
    ...
    fprintf(stderr, "\nUsage examples:\n");
    fprintf(stderr, "AtlasCreationTool.exe -halftexel -o Default Textures\\*.png\n");
    fprintf(stderr, "AtlasCreationTool.exe -integer -margin 2 -width 4096 -height 4096 -o MyAtlas *.jpg *.png cars\\wrc*.png d:\\opt\\logo.jpg\n");
    fprintf(stderr, "\n");
    fprintf(stderr, "AtlasCreationTool basis: Copyright (c) NVIDIA Corporation. All rights reserved.\n");
    fprintf(stderr, "D3D9X and MiniDX9 SDK: Copyright (c) Microsoft Corporation. All rights reserved.\n");
    fprintf(stderr, "Additional contributions Copyright (c) www.RallySimFans.hu team. All rights reserved.\n");
}
